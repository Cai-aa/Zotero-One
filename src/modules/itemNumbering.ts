...
export class ItemNumberingFactory {
  ...
  static async registerNumberingColumn() {
    try {
      ...
      if (columnExists) {
        ztoolkit.log("Item numbering column already exists, skipping registration");
      } else {
  ...
  private static isColumnRegistered(): boolean {
    try {
      ...
      for (let i = 0; i < tree.columns.length; i++) {
        ...
        if (column && (column.id === 'zotero-items-column-itemNumber' || column.element?.getAttribute('data-key') === 'itemNumber')) {
          return true;
        }
      }
  ...
  static async ensureColumnVisible() {
    try {
      ...
      if (itemsView.tree && itemsView.tree.columns) {
        ...
        if (!columnFound) {
          ztoolkit.log("Item numbering column not found in tree, will be auto-added");
        }
      }
    ...
    try {
      ...
      if (pane) {
        ...
        if (currentCollectionKey !== this.currentCollectionKey) {
          ztoolkit.log(`Collection changed from ${this.currentCollectionKey} to ${currentCollectionKey}, recalculating...`);
          this.currentCollectionKey = currentCollectionKey;
          this.recalculateNumbers();
        }
        
        // 从当前集合的编号映射中获取编号
        const collectionNumbers = this.collectionItemNumbers.get(currentCollectionKey);
  ...
  static async recalculateNumbers() {
    ...
    try {
      ...
      // 过滤出有效的文献条目
      const validItems = items.filter((item: Zotero.Item) => item.isRegularItem());
      
      ztoolkit.log(`Found ${validItems.length} valid items for numbering in collection: ${currentCollectionKey}`);

      // 为当前集合创建编号映射
      const numberMap = new Map<number, number>();
      validItems.forEach((item: Zotero.Item, index: number) => {
        numberMap.set(item.id, index + 1);
        ztoolkit.log(`Item ${item.id} assigned number ${index + 1} in collection ${currentCollectionKey}`);
      });
  ...
  private static refreshItemTree() {
    try {
      ...
      if (pane) {
        ...
        } else if (pane.itemsView && typeof pane.itemsView.tree?.invalidate === 'function') {
  ...
  private static getCurrentSortInfo(): { column: string; direction: string } {
    ...
    try {
      ...
      if (typeof itemsView.getSortField === 'function' && typeof itemsView.getSortDirection === 'function') {
   ...
   */
  static sortItems(items: Zotero.Item[], column: string, direction: 'asc' | 'desc') {
    ...
    items.sort((a, b) => {
      ...
      // 特殊处理创建者字段
      if (column === 'creator') {
        valA = a.getCreators().map(c => c.lastName || (c as any).name || '').join(', ');
        valB = b.getCreators().map(c => c.lastName || (c as any).name || '').join(', ');
      } else {
  ...
  static registerToolbarButton() {
    ...
    toolbarbutton.setAttribute("tooltiptext", getString("toolbar-button-tooltip"));
    toolbarbutton.setAttribute("oncommand", "Zotero.ZoteroOne.toggleNumbering();");
  ...
  private static async getCurrentViewItems(pane: any): Promise<Zotero.Item[]> {
    try {
      ...
      if (pane.itemsView && typeof pane.itemsView.getVisibleItems === 'function') {
  ...
  static saveColumnPosition() {
    try {
      ...
      for (let i = 0; i < tree.columns.length; i++) {
        const column = tree.columns.getColumnAt(i);
        if (column) {
          const dataKey = column.element?.getAttribute('data-key') || column.id?.replace('zotero-items-column-', '');
      ...
      if (itemNumberColumnIndex >= 0) {
        ztoolkit.log(`Saving item number column position: ${itemNumberColumnIndex}`);
  ...
  static restoreColumnPosition() {
    try {
      ...
      ztoolkit.log(`Attempting to restore column position to: ${savedPosition}`);
      ...
      if (currentItemNumberIndex >= 0 && currentItemNumberIndex !== savedPosition) {
        // 尝试移动列到保存的位置
        const itemNumberColumn = tree.columns.getColumnAt(currentItemNumberIndex);
        if (itemNumberColumn && typeof tree.columns.moveColumn === 'function') {
          ...
          if (savedPosition < tree.columns.length) {
            ...
            ztoolkit.log(`Moved item number column from ${currentItemNumberIndex} to ${savedPosition}`);
          }
        }
      }
    } catch (error) {
   ...
   * 比较两个条目用于排序
   */
  static compareItems(a: Zotero.Item, b: Zotero.Item, sortDirection: number): number {
  ...
  static restoreSortState() {
    try {
      ...
      if (savedColumn === "itemNumber" && savedDirection) {
        ztoolkit.log(`Restoring sort state: ${savedColumn}, direction: ${savedDirection}`);
  ...
  static handleItemChange(event: string, type: string, ids: number[]) {
    if (event === "add" || event === "modify" || event === "delete" || event === "select") {
      // 延时执行以确保界面更新完成
      setTimeout(() => {
        this.recalculateNumbers();
      }, 100);
